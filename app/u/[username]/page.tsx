...
const TAG_COLORS: Record<string, string> = {
  ...
  Documentation: 'bg-zinc-100 text-zinc-700 border-zinc-200 dark:bg-zinc-800 dark:text-zinc-300 dark:border-zinc-700',
...
export default function PublicProfilePage() {
  ...
  if (loading && !refreshing) {
    return (
      <div className="flex min-h-screen flex-col items-center justify-center gap-4 bg-stone-50 dark:bg-zinc-950">
        <img src="/logo.png" alt="DumpIt" className="h-10 w-10 animate-pulse object-contain" />
        <p className="text-sm font-semibold text-zinc-500 dark:text-zinc-400">Loading library...</p>
      </div>
    );
  }
  ...
  if (error === 'User not found') {
    return (
      <div className="flex min-h-screen flex-col bg-stone-50 dark:bg-zinc-950">
        <header className="border-b border-zinc-200/60 dark:border-zinc-800 bg-white/80 dark:bg-zinc-900/80 backdrop-blur-md px-6 py-4 flex items-center gap-3">
          <img src="/logo.png" alt="DumpIt" className="h-8 w-8 object-contain" />
          <span className="text-lg font-bold text-zinc-900 dark:text-white">DumpIt</span>
        </header>
        <div className="flex flex-1 flex-col items-center justify-center px-4">
          <div className="max-w-sm w-full text-center p-8 bg-white dark:bg-zinc-900 border border-zinc-200 dark:border-zinc-800 rounded-2xl shadow-xl">
            <div className="mx-auto mb-4 flex h-14 w-14 items-center justify-center rounded-full bg-red-50 dark:bg-red-950/30">
              <AlertCircle className="h-7 w-7 text-red-500" />
            </div>
            <h2 className="text-xl font-bold text-zinc-900 dark:text-white mb-2">User not found</h2>
            <p className="text-sm text-zinc-500 dark:text-zinc-400 mb-6">
              No public library found for{' '}
              <span className="font-semibold text-zinc-700 dark:text-zinc-200">@{username}</span>.
            </p>
            <button
              onClick={() => router.push('/')}
  ...
  return (
    <div className="min-h-screen bg-stone-50 dark:bg-zinc-950 flex flex-col">
      ...
      {/* Navbar */}
      <header className="sticky top-0 z-20 border-b border-zinc-200/70 dark:border-zinc-800/80 bg-white/80 dark:bg-zinc-900/80 backdrop-blur-md">
        <div className="mx-auto flex max-w-5xl items-center justify-between px-4 py-3 sm:px-6">
          ...
          <div className="flex items-center gap-2">
            <button
              ...
              disabled={refreshing}
              className="inline-flex items-center gap-1.5 rounded-lg border border-zinc-200 dark:border-zinc-700 bg-white dark:bg-zinc-800 px-3 py-1.5 text-xs font-semibold text-zinc-600 dark:text-zinc-300 hover:bg-zinc-50 dark:hover:bg-zinc-700 transition-colors disabled:opacity-50"
            >
      ...
      {/* Hero / Profile banner */}
      <div className="relative overflow-hidden border-b border-zinc-200/60 dark:border-zinc-800/80 bg-white dark:bg-zinc-900">
        ...
        <div className="relative mx-auto max-w-5xl px-4 py-10 sm:px-6 sm:py-14">
          <div className="flex flex-col items-start gap-5 sm:flex-row sm:items-center sm:justify-between">
            <div className="flex items-center gap-5">
              <div className="relative flex h-16 w-16 shrink-0 items-center justify-center rounded-2xl bg-gradient-to-br from-blue-500 to-violet-600 text-2xl font-bold uppercase text-white shadow-lg shadow-blue-500/20 ring-4 ring-white dark:ring-zinc-900">
            ...
            <div className="flex items-center gap-3 rounded-xl border border-zinc-200 dark:border-zinc-800 bg-stone-50 dark:bg-zinc-950 px-5 py-3 shrink-0">
              <div className="text-center">
                <div className="text-2xl font-bold text-blue-600 dark:text-blue-400">{resources.length}</div>
                <div className="text-[11px] font-semibold text-zinc-500 dark:text-zinc-400 uppercase tracking-wide">Public links</div>
              </div>
              {selectedTag && (
                <>
                  <div className="h-8 w-px bg-zinc-200 dark:bg-zinc-800" />
                  <div className="text-center">
                    <div className="text-2xl font-bold text-zinc-700 dark:text-zinc-200">{filteredResources.length}</div>
                    <div className="text-[11px] font-semibold text-zinc-500 dark:text-zinc-400 uppercase tracking-wide">Filtered</div>
                  </div>
                </>
              )}
            </div>
      ...
      <main className="mx-auto w-full max-w-5xl flex-1 px-4 py-8 sm:px-6 space-y-5">
        ...
        <div className="space-y-3">
          <div className="relative">
            ...
            <input
              ...
              className="w-full rounded-xl border border-zinc-200 dark:border-zinc-800 bg-white dark:bg-zinc-900 py-3 pl-10 pr-4 text-sm outline-none placeholder-zinc-400 dark:placeholder-zinc-500 transition-all focus:border-blue-500 focus:ring-2 focus:ring-blue-500/20 dark:text-white"
            />
          </div>
        ...
        {filteredResources.length === 0 ? (
          <div className="flex flex-col items-center justify-center rounded-2xl border border-zinc-200/60 dark:border-zinc-800 bg-white dark:bg-zinc-900 py-16 text-center shadow-sm">
            <div className="mx-auto mb-4 flex h-14 w-14 items-center justify-center rounded-full bg-zinc-100 dark:bg-zinc-800">
              <BookOpen className="h-6 w-6 text-zinc-400 dark:text-zinc-500" />
            </div>
            ...
            {(searchQuery || selectedTag) && (
              <button
                ...
                className="mt-4 rounded-lg border border-zinc-200 dark:border-zinc-700 px-4 py-2 text-xs font-semibold text-zinc-600 dark:text-zinc-300 hover:bg-zinc-50 dark:hover:bg-zinc-800 transition-colors"
              >
                Clear filters
              </button>
            )}
          ...
          <div className="grid gap-3 sm:grid-cols-2 xl:grid-cols-3">
            {filteredResources.map((resource) => (
              <a
                ...
                className="group flex flex-col rounded-2xl border border-zinc-200/70 dark:border-zinc-800/80 bg-white dark:bg-zinc-900 p-4 shadow-sm hover:border-blue-300 dark:hover:border-blue-800 hover:shadow-md hover:shadow-blue-500/5 transition-all duration-200"
              >
                <div className="flex items-center justify-between gap-2 mb-3">
                  <div className="flex items-center gap-2 min-w-0">
                    ...
                    />
                    <span className="truncate text-[11px] font-semibold uppercase tracking-wide text-zinc-400 dark:text-zinc-500">
                ...
                <h3 className="line-clamp-2 text-sm font-bold text-zinc-900 dark:text-white group-hover:text-blue-600 dark:group-hover:text-blue-400 transition-colors leading-snug mb-2">
                  {resource.title || 'Untitled Resource'}
                </h3>

                {resource.note && (
                  <p className="line-clamp-3 text-xs leading-relaxed text-zinc-500 dark:text-zinc-400 flex-1 mb-3">
                    {resource.note}
                  </p>
                )}

                <div className="mt-auto flex items-center justify-between gap-2 pt-3 border-t border-zinc-100 dark:border-zinc-800">
      ...
      {/* Footer */}
      <footer className="border-t border-zinc-200/60 dark:border-zinc-800/80 bg-white dark:bg-zinc-900 mt-auto">
