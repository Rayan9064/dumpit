...
const statusLabel: Record<string, { label: string; style: string }> = {
  ...
  roadmap: { label: 'Roadmap', style: 'bg-zinc-100 text-zinc-500 dark:bg-zinc-800 dark:text-zinc-400' },
}

const ContentTypes = () => {
  return (
    <section id="content-types" className="scroll-mt-20 border-b border-zinc-200 bg-stone-50 py-24 dark:border-zinc-800 dark:bg-[#0b0f17]">
          ...
          {types.map((type) => {
            ...
            return (
              <article key={type.label} className="app-panel p-5">
                ...
                <h3 className="mb-1.5 font-bold text-zinc-950 dark:text-white">{type.label}</h3>
                <p className="text-sm leading-6 text-zinc-600 dark:text-zinc-300">{type.description}</p>
              </article>
            )
          })}
        </div>
